...
void spendere_punti(Personaggio& personaggio, int &punti_da_spendere) {
    while (punti_da_spendere > 0) {
        ...
        cout << "Punti rimasti: " << punti_da_spendere << endl;
        cout << "1. Forza: " << personaggio.forza << " (mod: " << personaggio.modificatori[Personaggio::FORZA] << ")" << endl;
        cout << "2. Destrezza: " << personaggio.destrezza << " (mod: " << personaggio.modificatori[Personaggio::DESTREZZA] << ")" << endl;
        cout << "3. Costituzione: " << personaggio.costituzione << " (mod: " << personaggio.modificatori[Personaggio::COSTITUZIONE] << ")" << endl;
        cout << "4. Intelligenza: " << personaggio.intelligenza << " (mod: " << personaggio.modificatori[Personaggio::INTELLIGENZA] << ")" << endl;
        cout << "5. Saggezza: " << personaggio.saggezza << " (mod: " << personaggio.modificatori[Personaggio::SAGGEZZA] << ")" << endl;
        cout << "6. Carisma: " << personaggio.carisma << " (mod: " << personaggio.modificatori[Personaggio::CARISMA] << ")" << endl;

        int scelta;
        while (true) {
            cout << "Scegli l'attributo da incrementare (1-6): ";
            if (!(cin >> scelta) || scelta < 1 || scelta > 6) {
                cout << "Input non valido! Inserisci un numero da 1 a 6." << endl;
                cin.clear();
        ...
        while (true) {
            ...
            if (!(cin >> punti_spesi) || punti_spesi < 1) {
                cout << "Input non valido! Inserisci un numero positivo." << endl;
                cin.clear();
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
                continue;
            }
        ...
        // Aumenta attributo e restituisce i punti effettivamente spesi
        ...
        // Aggiorna i punti totali rimanenti
        punti_da_spendere -= spesi_effettivi;

        // Se l'utente ha tentato di spendere più punti del necessario, restituisci il resto
        if (spesi_effettivi < punti_spesi) {
            int resto = punti_spesi - spesi_effettivi;
            cout << "Hai raggiunto il massimo per questo attributo. Restano " << resto << " punti da spendere." << endl;
            punti_da_spendere += resto; // li rimetti nei punti totali
        }

        cout << "----------------------------------------" << endl;
    }

    cout << "Hai speso tutti i punti disponibili!" << endl;
}